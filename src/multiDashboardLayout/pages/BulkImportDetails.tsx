...
const mockListings: Listing[] = [{
  id: '1',
  name: 'Webmart software solution...',
  zipCode: '431105'
}, {
  id: '2',
  name: 'Webmart software solution...',
...
const ListingSidebar = ({
  ...
  const filteredListings = listings.filter(listing => listing.name.toLowerCase().includes(searchQuery.toLowerCase()) || listing.zipCode.includes(searchQuery));
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="flex gap-4">
            <div className="flex-1">
              <Input placeholder="Search by post title" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full" />
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredPosts.length === 0 ? <TableRow>
                      <TableCell colSpan={6} className="h-24 text-center text-muted-foreground">
                    ...
                    </TableRow> : filteredPosts.map(post => <TableRow key={post.id} className="border-border">
                        <TableCell className="text-muted-foreground">
                          {post.id}
                        </TableCell>
                        <TableCell>
                          <img src={post.image} alt={post.title} className="w-12 h-12 rounded-lg object-cover border border-border" />
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Button variant="ghost" size="sm" onClick={() => handleViewPost(post.id)} className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground">
                              <Eye className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => handleDeletePost(post.id)} className="h-8 w-8 p-0 text-muted-foreground hover:text-destructive">
