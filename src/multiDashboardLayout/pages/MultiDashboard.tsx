...
export const MultiDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (profileData?.dashboardFilterType) {
      const savedType =
        DASHBOARD_ID_TO_TYPE_MAPPING[profileData.dashboardFilterType as keyof typeof DASHBOARD_ID_TO_TYPE_MAPPING];
  ...
  const { data: groupListData, isLoading: groupListLoading, error: groupListError } = useGroupLists("");
  ...
  const defaultParams = useMemo(
    ...
    [currentPage, itemsPerPage, debouncedSearchTerm, selectedCategory, selectedState, selectedGroup],
  );
  ...
  const defaultDashboardQuery = useDashboardData(defaultParams, dashboardType === "default");
  const insightsDashboardQuery = useInsightsDashboardData(insightsParams, dashboardType === "insight");
  const reviewDashboardQuery = useReviewDashboardData(reviewParams, dashboardType === "review");
  const locationDashboardQuery = useLocationDashboardData(locationParams, dashboardType === "location");
  const postDashboardQuery = usePostsDashboardData(postParams, dashboardType === "post");
  ...
  return (
    <TooltipProvider>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                >
                  ...
                  <SelectContent className="bg-background border">
                    <SelectItem value="default">{t("dashboard.types.default")}</SelectItem>
                    <SelectItem value="insight">{t("dashboard.types.insight")}</SelectItem>
                    <SelectItem value="review">{t("dashboard.types.review")}</SelectItem>
                    <SelectItem value="location">{t("dashboard.types.location")}</SelectItem>
                    <SelectItem value="post">{t("dashboard.types.post")}</SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-2">
                ...
                >
                  <SelectTrigger className="w-full sm:w-40 bg-background" name="selectCategory">
                    <SelectValue
                      placeholder={categoryStateLoading ? t("status.loading") : t("dashboard.filters.allCategories")}
                    />
                  </SelectTrigger>
                  <SelectContent className="z-50 bg-background">
                    <SelectItem value="all">{t("dashboard.filters.allCategories")}</SelectItem>
                ...
                >
                  <SelectTrigger className="w-full sm:w-40 bg-background" name="selectStats">
                    <SelectValue
                      placeholder={categoryStateLoading ? t("status.loading") : t("dashboard.filters.allStates")}
                    />
                  </SelectTrigger>
                  <SelectContent className="z-50 bg-background">
                    <SelectItem value="all">{t("dashboard.filters.allStates")}</SelectItem>
                    {!categoryStateLoading &&
                      !categoryStateError &&
                ...
                <Select value={selectedGroup} onValueChange={handleGroupFilterChange} disabled={groupListLoading}>
                  <SelectTrigger className="w-full sm:w-40 bg-background" name="selectGroup">
                    <SelectValue
                      placeholder={groupListLoading ? t("status.loading") : t("dashboard.filters.allGroups")}
                    />
                  </SelectTrigger>
                  <SelectContent className="z-50 bg-background">
                ...
                {dashboardType === "review" && (
                  <Select value={reviewFilter} onValueChange={handleReviewFilterChange}>
                    <SelectTrigger className="w-full sm:w-52">
                      <SelectValue placeholder={t("dashboard.filters.reviewFilter")} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="0">{t("dashboard.filters.allReviews")}</SelectItem>
                      <SelectItem value="1">{t("dashboard.filters.unrespondedReview")}</SelectItem>
                      <SelectItem value="2">{t("dashboard.filters.unrespondedARE")}</SelectItem>
                      <SelectItem value="3">{t("dashboard.filters.unrespondedDNR")}</SelectItem>
                      <SelectItem value="4">{t("dashboard.filters.excludeARE")}</SelectItem>
                      <SelectItem value="5">{t("dashboard.filters.excludeDNR")}</SelectItem>
                      <SelectItem value="6">{t("dashboard.filters.excludeARE_DNR")}</SelectItem>
                    </SelectContent>
                  </Select>
                )}
                  ...
                  <>
                    <Select value={postStatus} onValueChange={handlePostStatusChange}>
                      <SelectTrigger className="w-full sm:w-48">
                        <SelectValue placeholder={t("dashboard.filters.postStatus")} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">{t("dashboard.filters.allPosts")}</SelectItem>
                        <SelectItem value="scheduled">{t("dashboard.filters.scheduledPost")}</SelectItem>
                        <SelectItem value="live">{t("dashboard.filters.livePost")}</SelectItem>
                        <SelectItem value="failed">{t("dashboard.filters.failedPost")}</SelectItem>
                      </SelectContent>
                    </Select>
                    <DateRangePicker
                      date={dateRange}
            ...
            {isDashboardLoading ? (
              ...
              <div className="text-center py-8">
                <p className="text-gray-500">
                  {dashboardType === "post" ? t("errors.postsLoad") : t("errors.listingsLoad")}
                  {/* Failed to load{" "}
              ...
              viewMode === "grid" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {posts.map((post) => (
                    <PostCard key={post.id} post={transformPostDashboardPost(post)} />
                  ))}
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {listing.isSync === 1 ? (
                        ...
                        <div className="mb-5">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            <div className="flex items-center gap-2 p-2 bg-muted/50 rounded">
                              ...
                              <span>
                                {listing.customer_actions?.website_clicks || 0} {t("listing.clicks")}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 p-2 bg-muted/50 rounded">
                              <MapPin className="w-3 h-3 text-green-500" />
                              <span>
                                {listing.customer_actions?.direction_requests || 0} {t("listing.directions")}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 p-2 bg-muted/50 rounded">
                              <Phone className="w-3 h-3 text-orange-500" />
                              <span>
                                {listing.customer_actions?.phone_calls || 0} {t("listing.calls")}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 p-2 bg-muted/50 rounded">
                              <MessageSquare className="w-3 h-3 text-purple-500" />
                              <span>
                                {listing.customer_actions?.messages || 0} {t("listing.messages")}
                              </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <>
                        {/* Rating Section */}
                        <div className="mb-4">
                          <h5 className="text-sm font-medium text-muted-foreground mb-2">{t("listing.reviewStats")}</h5>
                          <div className="flex items-center gap-2 mb-3">
                            <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                            <span
                              className={`font-bold text-sm ${getStatusColor(listing.avgRating || listing.rating)}`}
                            >
                        ...
                        <div className="mb-4 p-3 bg-muted/50 rounded-lg">
                          <div className="grid grid-cols-2 text-sm">
                            <div className="text-center border-r border-gray-300">
                              <span className="text-muted-foreground font-medium">{t("listing.reviewsReplies")}</span>
                              <p className="font-semibold text-foreground">
                                {listing.reviewCount || 0} / {listing.replyCount || 0}
                              </p>
                            </div>
                            <div className="text-center">
                              <span className="text-muted-foreground font-medium">{t("listing.autoReply")}:</span>
                      ...
                      <>
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            ></div>
                            <span className="font-bold text-sm text-foreground">{listing.status || "Active"}</span>
                          </div>
                        </div>
                        ...
                        <div className="mb-4 p-3 bg-muted/50 rounded-lg">
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div>
                              <span className="text-muted-foreground font-medium">{t("listing.visibilityLabel")}</span>
                              <p className="font-semibold text-foreground">{listing.visibility || "Public"}</p>
                            </div>
                            <div>
                              <span className="text-muted-foreground font-medium">{t("listing.completeLabel")}</span>
                        ...
                        <div className="mb-5 space-y-2">
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-muted-foreground font-medium">{t("listing.lastUpdated")}</span>
                      ...
                      <>
                        {/* Rating Section */}
                        <div className="mb-4">
                          <h5 className="text-sm font-medium text-muted-foreground mb-2">{t("listing.avgRating")}</h5>
                          <div className="flex items-center gap-2 mb-3">
                            <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                        ...
                        <div className="mb-5 space-y-2">
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-muted-foreground font-medium">{t("listing.lastPost")}:</span>
                            <span className="text-foreground font-medium">
                              {listing.lastPost?.includes("No post") ? t("listing.nopost") : listing.lastPost}
                            </span>
                          </div>
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-muted-foreground font-medium">{t("listing.upcoming")}:</span>
                            <span className="text-foreground font-medium">
                              {listing.upcomingPost?.includes("No post") ? t("listing.nopost") : listing.upcomingPost}
                            </span>
                          </div>
                        </div>
                      </>
                    ...
                    <div className="flex justify-end mt-auto">
                      <Button
                        variant="default"
                        size="sm"
                        onClick={() => navigate(`/location-dashboard/${listing.listingId || listing.id}`)}
                      ...
                      <div className="flex items-center gap-4 min-w-0 flex-1">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            {listing.isSync === 1 && <Loader2 className="w-3 h-3 animate-spin text-primary" />}
                          </div>
                        ...
                        {dashboardType === "insight" ? (
                          <>
                            <div className="text-center min-w-16 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.totalViews")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-blue-600">{listing.visibility?.search_views || 0}</div>
                              <div className="text-muted-foreground">{t("listing.Search")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-green-600">{listing.visibility?.maps_views || 0}</div>
                              <div className="text-muted-foreground"> {t("listing.Maps")}</div>
                            </div>
                            <div className="text-center min-w-20 flex-shrink-0">
                              <div className="font-semibold text-cyan-600">
                                {listing.customer_actions?.direction_requests || 0}
                              </div>
                              <div className="text-muted-foreground">{t("listing.directions")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-orange-600">
                                {listing.customer_actions?.phone_calls || 0}
                              </div>
                              <div className="text-muted-foreground"> {t("listing.calls")}</div>
                            </div>
                            <div className="text-center min-w-20 flex-shrink-0">
                              <div className="font-semibold text-pink-600">
                                {listing.customer_actions?.messages || 0}
                              </div>
                              <div className="text-muted-foreground">{t("listing.messages")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-purple-600">
                                {listing.customer_actions?.website_clicks || 0}
                              </div>
                              <div className="text-muted-foreground">{t("listing.clicks")}</div>
                            </div>
                          </>
                        ) : dashboardType === "review" ? (
                          <>
                            ...
                            <div className="text-center min-w-20 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.reviewsVsReplies")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-green-600">{listing.sentiment?.positive || 0}</div>
                              <div className="text-muted-foreground">{t("listing.positive")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-gray-600">{listing.sentiment?.neutral || 0}</div>
                              <div className="text-muted-foreground">{t("listing.neutral")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-red-600">{listing.sentiment?.negative || 0}</div>
                              <div className="text-muted-foreground">{t("listing.negative")}</div>
                            </div>
                            <div className="text-center min-w-20 flex-shrink-0">
                              <div
                              ...
                              <div className="text-muted-foreground">{t("listing.autoReply")}</div>
                            </div>
                          </>
                        ) : dashboardType === "location" ? (
                          <>
                            <div className="text-center min-w-16 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.photos")}</div>
                            </div>
                            <div className="flex items-center gap-2 min-w-16 flex-shrink-0">
                              <Star className="w-3 h-3 text-yellow-500 fill-yellow-500" />
                            ...
                            <div className="text-center min-w-20 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.phone")}</div>
                            </div>
                            <div className="text-center min-w-16 flex-shrink-0">
                              <div className="font-semibold text-foreground truncate">
                                {listing.state || t("listing.na")}
                              </div>
                              <div className="text-muted-foreground">{t("listing.stateLabel")}</div>
                            </div>
                            {listing.map && (
                              <div className="text-center min-w-16 flex-shrink-0">
                                <button
                                ...
                                >
                                  ...
                                  <div className="text-muted-foreground text-xs">{t("listing.maps")}</div>
                                </button>
                              </div>
                            )}
                            {listing.website && (
                              <div className="text-center min-w-16 flex-shrink-0">
                                ...
                                >
                                  ...
                                  <div className="text-muted-foreground text-xs">{t("listing.website")}</div>
                                </button>
                              </div>
                            )}
                          </>
                          ...
                          <>
                            <div className="text-center min-w-20 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.starRating")}</div>
                            </div>
                            <div className="text-center min-w-20 flex-shrink-0">
                            ...
                            <div className="text-center min-w-20 flex-shrink-0">
                              ...
                              <div className="text-muted-foreground">{t("listing.lastPost")}</div>
                            </div>
                            <div className="text-center min-w-20 flex-shrink-0">
                              <div className="font-semibold text-foreground truncate">
                                {listing.upcomingPost?.includes("No post") ? t("listing.nopost") : listing.upcomingPost}
                              </div>
                              <div className="text-muted-foreground">{t("listing.upcoming")}</div>
                            </div>
                          </>
                        )}
                      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6">
                <div className="text-sm text-muted-foreground text-center sm:text-left">
                  {
                    dashboardType === "post"
                      ? t("pagination.showing.posts", {
                          ...
                          to: Math.min(pagination.currentPage * itemsPerPage, (pagination as any).totalPosts),
                      ...
                      : // `Showing ${
                        ...
                        t("pagination.showing.listings", {
                          from: (pagination.currentPage - 1) * (pagination as any).resultsPerPage + 1,
                          to: Math.min(
                            pagination.currentPage * (pagination as any).resultsPerPage,
                ...
                <div className="flex items-center justify-center sm:justify-end gap-2">
                  ...
                  <Button
                    ...
                    disabled={currentPage === pagination.totalPages || isDashboardLoading}
                  >
      ...
      {/* Copy URL Modal */}
      <CopyUrlModal open={showCopyUrlModal} onOpenChange={setShowCopyUrlModal} reportUrl={generatedReportUrl} />

      {/* Export Reviews Modal */}
      <ExportReviewsModal open={showExportModal} onOpenChange={setShowExportModal} />
    </TooltipProvider>
  );
};
