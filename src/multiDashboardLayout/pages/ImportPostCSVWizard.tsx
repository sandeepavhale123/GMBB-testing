...
"Another Sample","Another sample description","event","2024-01-16"`;
  ...
  const renderStepIndicator = () => <div className="w-64 border-r border-gray-200 p-6 hidden lg:block min-h-[80vh]">
      ...
      <div className="space-y-4 p-3 bg-gray-50">
        {steps.map(step => <div key={step.number} className="flex items-center gap-3">
            <div className="">
              {step.completed ? <div className="flex items-center justify-center w-8 h-8 rounded-full bg-success"><Check className="w-5 h-5 text-white" /></div> : currentStep === step.number ? <div className="w-8 h-8 rounded-full bg-gray-300"></div> : <div className="w-8 h-8 rounded-full bg-gray-300"></div>}
            </div>
