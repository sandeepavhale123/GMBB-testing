  ...
  const renderStepIndicator = () => <div className="w-64 border-r border-gray-200 p-6 hidden lg:block min-h-[80vh]">
      ...
      <div className="space-y-4">
        {steps.map(step => <div key={step.number} className="flex items-center gap-3 bg-gray-50 p-3">
            <div className="">
              {step.completed ? <div className="flex items-center justify-center w-6 h-6 rounded-full bg-success"><Check className="w-4 h-4 text-white" /></div> : currentStep === step.number ? <div className="w-6 h-6 rounded-full bg-blue-100"></div> : <div className="w-6 h-6 rounded-full bg-gray-300"></div>}
            </div>
