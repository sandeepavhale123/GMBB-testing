...
export const QuickWinsCard: React.FC<QuickWinsCardProps> = ({
  ...
  const { t } = useI18nNamespace("Dashboard/quickWinsCard");
  ...
  const sortedQuickWins = [...quickwins].sort(
    ...
    <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-base sm:text-lg font-semibold">
              {t("quickWinsTitle")} ({quickwins.length})
            </CardTitle>
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">
              {t("quickWinsDescription")}
            </p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3 sm:space-y-4">
        {sortedQuickWins.length > 0 ? (
          sortedQuickWins.slice(0, 3).map((win) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${getPriorityColor(
                    win.frequency
                  )}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    ...
                    title={win.task_description}
                  >
                    {win.task_description.length > 25
                      ? win.task_description.slice(0, 25) + "..."
                      : win.task_description}
                  </p>
                </div>
                <Button
                  size="sm"
          ...
          <div className="text-center py-4">
            <p className="text-sm text-muted-foreground">{t("noQuickWins")}</p>
          </div>
        )}
        <Button
          variant="outline"
