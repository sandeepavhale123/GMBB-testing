...
export const PostsContent: React.FC<PostsContentProps> = ({
  ...
  const {
    t
  } = useI18nNamespace("Post/postsContent");
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? (error as any)?.response?.data?.message || error.message : t("toast.deleteFailedDesc"),
        variant: "destructive"
      });
    }
  };
  ...
  return <>
        ...
        <div className="flex items-center gap-4">
          ...
          {isSelectionMode && <>
              <Button variant="outline" onClick={handleSelectAll} size="sm">
                {selectedPosts.size === posts.length ? t("deselectAll") : t("selectAll")}
              </Button>
              ...
              {selectedPosts.size > 0 && <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant="destructive" size="sm" disabled={deleteLoading}>
                      ...
                      {deleteLoading ? t("deleting") : t("deleteSelected", {
                  count: selectedPosts.size
                })}
                    </Button>
                  </AlertDialogTrigger>
      ...
      {viewMode === "grid" ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {posts.map(post => <PostCard key={post.id} post={post} isSelectionMode={isSelectionMode} isSelected={selectedPosts.has(post.id)} onSelect={handleSelectPost} onClonePost={onClonePost} />)}
        </div> : <div className="bg-white rounded-lg border divide-y p-4">
          {posts.map(post => <PostListItem key={post.id} post={post} onClonePost={onClonePost} isSelectionMode={isSelectionMode} isSelected={selectedPosts.has(post.id)} onSelectionChange={handleSelectPost} />)}
        </div>}
