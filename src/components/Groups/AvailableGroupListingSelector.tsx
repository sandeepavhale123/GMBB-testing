...
import { useGetAvailableGroupListingsMutation, AvailableListingLocation } from '@/api/listingsGroupsApi';
...
export const AvailableGroupListingSelector: React.FC<AvailableGroupListingSelectorProps> = ({
  groupId,
  selectedListings,
  onListingsChange,
  error
  ...
  const [locationsOpen, setLocationsOpen] = useState<{ [key: string]: boolean }>({});
  const dropdownRef = useRef<HTMLDivElement>(null);

  const [getAvailableListings, { isLoading }] = useGetAvailableGroupListingsMutation();
    ...
    try {
      ...
      if (response.code === 200 && response.data?.locationGroups) {
        ...
        Object.entries(response.data.locationGroups).forEach(([email, locations]) => {
          ...
          const availableLocations = locations.filter((location: AvailableListingLocation) => !location.alreadyAdded);
  ...
  const getSelectedOptions = (): ListingOption[] => {
    return options.filter(option => selectedListings.includes(parseInt(option.id)));
  };
  ...
  const handleDeselectAllLocations = () => {
    ...
    const newSelected = selectedListings.filter(id => !filteredIds.includes(id));
    onListingsChange(newSelected);
  };

  const areAllLocationsSelected = filteredOptions.length > 0 && 
    filteredOptions.every(option => selectedListings.includes(parseInt(option.id)));

  return (
    <div className="space-y-2">
      <label className="text-sm font-medium">Available Listings</label>
      <div className="relative" ref={dropdownRef}>
        ...
        {open && (
          <div className="absolute z-50 w-full mt-1 bg-background border border-border rounded-md shadow-lg max-h-60 overflow-hidden">
            ...
            <div className="max-h-48 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="p-3 text-center text-muted-foreground">
                  {error ? error : searchTerm ? 'No listings found.' : 'No available listings to add.'}
                </div>
              ) : (
                <>
                  {filteredOptions.length > 0 && (
                    <div className="p-2 border-b border-border">
                      ...
                      >
                        {areAllLocationsSelected ? 'Deselect All' : 'Select All'} ({filteredOptions.length})
                      </Button>
                    </div>
                  )}

                  {Object.entries(groupedOptions).map(([email, listings]) => (
                    <div key={email} className="border-b border-border last:border-b-0">
                      <button
                        type="button"
                        className="w-full p-3 text-left hover:bg-muted transition-colors flex items-center justify-between"
                        onClick={() => setLocationsOpen(prev => ({ ...prev, [email]: !prev[email] }))}
                      >
                        <span className="font-medium text-sm">{email || 'Unknown'}</span>
                        <div className="flex items-center space-x-2">
                          <span className="text-xs text-muted-foreground">
                            {listings.length} listing{listings.length !== 1 ? 's' : ''}
                          </span>
                          <ChevronDown 
                            className={`h-4 w-4 transition-transform ${locationsOpen[email] ? 'rotate-180' : ''}`} 
                          />
                        </div>
                      </button>
                      ...
                      {locationsOpen[email] && (
                        <div className="bg-muted/50">
                          {listings.map((listing) => {
                            const isSelected = selectedListings.includes(parseInt(listing.id));
                            return (
                              <div
                                key={listing.id}
            ...
            {selectedListings.length === 0 && Object.keys(groupedOptions).length > 0 && !error && (
              <div className="p-3 border-t border-border bg-muted/50 text-center text-xs text-muted-foreground">
                Select at least one listing to add to the group
              </div>
            )}
          </div>
        )}
