...
export const MediaPage: React.FC = () => {
  const {
    t
  } = useI18nNamespace("Media/mediaPage");
  const {
    toast
  } = useToast();
  const {
  ...
  const confirmDeleteMedia = async () => {
    ...
    try {
      ...
      toast({
        title: t("mediaPage.toast.errorTitle"),
        description: error?.response?.data?.message || error.message || t("mediaPage.toast.errorDeleteRetry"),
        variant: "destructive"
      });
    } finally {
      setIsDeleting(false);
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmDeleteMedia} disabled={isDeleting} className="bg-red-600 hover:bg-red-700">
              {isDeleting ? t("mediaPage.deleteDialog.deleting") : t("mediaPage.deleteDialog.delete")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
