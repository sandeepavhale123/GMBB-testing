...
import { LayoutDashboard, FileText, Image, Images, BarChart3, MapPin, Star, Building, Settings, Crown, Sparkles, MessageCircleQuestion, FileBarChart, Bot, BookOpen, ChevronDown, ChevronRight, Search, TrendingUp, ArrowLeft } from "lucide-react";
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const trialPlan = profileData?.planName?.toLowerCase() === "7$ for 7-day trial" || profileData?.planName?.toLowerCase() === "trial" || profileData?.planId === "50";
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-3 pb-4 pt-0">
          <ScrollArea className="px-3 py-4 ">
            <nav className="space-y-2">
              {menuItems.filter(item => !(item.id === "settings" && shouldHideForRole())).filter(item => !(item.id === "main-dashboard" && profileData?.dashboardType === 0)).map(item => {
