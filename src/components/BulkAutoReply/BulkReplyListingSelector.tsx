...
export const BulkReplyListingSelector: React.FC<
  BulkReplyListingSelectorProps
> = ({
  selectedListings,
  onListingsChange,
  onOptionsChange,
  ...
  const { t } = useI18nNamespace("BulkAutoReply/bulkReplyListingSelector");
  ...
  const [getBulkReplyDetails, { isLoading }] = useGetBulkReplyDetailsMutation();
  ...
  const fetchListings = async () => {
    try {
      const response = await getBulkReplyDetails(
        projectId ? { projectId } : {}
      ).unwrap();
      ...
      const locationOptions: BulkReplyListingOption[] =
        response.data.locationLists.map((location: BulkReplyLocationsList) => ({
          id: location.id,
    ...
    <div className="space-y-3">
      ...
      <Collapsible open={open} onOpenChange={setOpen}>
        ...
        <CollapsibleContent className="mt-2">
          <div
            ref={containerRef}
            className="border rounded-md bg-background shadow-lg z-50 relative"
          >
            <div className="p-3">
              {/* Search Input */}
