...
import { BarChart3, Star, MapPin, Heart, Image, LogOut, Search, Bell, User, Sun, Moon, Target, FileText, X, Menu } from "lucide-react";
...
export const PublicReportDashboardLayout: React.FC<PublicReportDashboardLayoutProps> = ({
  ...
  const {
    data: brandingData,
    isLoading
  } = usePerformanceBrandingReport(token);
  ...
  const sidebarItems = allSidebarItems.filter(item => (visibleSections || []).includes(item.name));
  ...
  return <TooltipProvider>
      <div className="min-h-screen bg-white flex relative">
        ...
        `}>
          ...
          <div className="mb-8">
            {/* <img
             ...
             /> */}
            {branding?.company_logo ? <img src={branding?.company_logo} alt="Company Logo" className="w-12 h-12 rounded-xl shadow-lg object-cover" /> : <img src={lightLogo} alt="Default Logo" className="w-12 h-12 rounded-xl shadow-lg object-cover" />}
          </div>
        ...
        <div className={`flex-1 flex flex-col transition-all duration-300 ${isMobile ? "ml-0" : "ml-24"}`}>
          {/* Dark Header */}
          <header className="text-white h-[350px] z-10 relative sm:h-[250px]" style={{
          background: `linear-gradient(135deg, hsl(var(--primary-gradient-from)), hsl(var(--primary-gradient-via)), hsl(var(--primary-gradient-from) / 0.8))`
            ...
            {/* Mobile Menu Button */}
            {isMobile && <button onClick={() => setSidebarOpen(!sidebarOpen)} className="absolute top-4 left-4 p-2 bg-white/20 rounded-lg backdrop-blur-sm z-20">
            ...
            paddingBottom: "50px"
              ...
              <div className={`flex items-center space-x-4`}>
                {logo ? <img src={logo} alt="Business Logo" className={`rounded-lg object-cover ${isMobile ? "w-12 h-12" : "w-16 h-16"}`} /> : <div className={`bg-white rounded-lg flex items-center justify-center ${isMobile ? "w-12 h-12" : "w-16 h-16"}`}>
                ...
                <div className={`flex flex-col ${isMobile ? " space-y-1" : ""}`}>
                  <h1 className={`font-bold text-white ${isMobile ? "text-base" : "text-2xl"}`}>
                    {listingName}
                  </h1>
                  <p className={`text-white  ${isMobile ? "text-xs leading-tight max-w-[280px]" : "text-lg"}`}>
                    {address}
                  </p>
                </div>
              </div>
                ...
                <div className="text-white">
                  ...
                  <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 flex justify-between flex-col gap-8 lg:flex-row lg:gap-6">
                    <div className={`flex items-center ${isMobile ? "space-x-4" : "space-x-4"}`}>
                      {branding?.company_logo ? <img src={branding?.company_logo} alt="Company Logo" className="w-20 h-20 rounded-lg object-cover" /> : <div className="w-20 h-20 bg-white/20 rounded-lg flex items-center justify-center">
