...
export const GeoRankingReportPage: React.FC = () => {
  ...
  // Debug logging
  console.log("📍 GeoRankingReportPage: URL listingId:", listingId);
  console.log("📍 GeoRankingReportPage: numericListingId:", numericListingId);
  console.log("📍 GeoRankingReportPage: listings:", listings);
  console.log("📍 GeoRankingReportPage: listings loading:", listingsLoading);
  console.log(
    "📍 GeoRankingReportPage: selectedListing from context:",
    selectedListing
  );
  // Get current listing - fix the comparison logic to handle both string and number IDs
  const currentListing = listings.find((listing) => {
    console.log(
      "📍 Comparing listing.id:",
      listing.id,
      "with numericListingId:",
      numericListingId
    );
    console.log(
      "📍 Types - listing.id:",
    ...
    const targetIdStr = String(numericListingId);

    return listingIdStr === targetIdStr;
  });

  console.log("📍 GeoRankingReportPage: currentListing found:", currentListing);
  // Set the selected listing in context when currentListing is found
  useEffect(() => {
    console.log(
      "📍 GeoRankingReportPage: useEffect for selectedListing triggered"
    );
    ...
    // Only set if we have a currentListing and it's different from selectedListing
    if (currentListing && !listingsLoading) {
      const shouldUpdate =
        !selectedListing || selectedListing.id !== currentListing.id;
      console.log("📍 Should update selectedListing:", shouldUpdate);

      if (shouldUpdate) {
        console.log("📍 Setting selectedListing in context:", currentListing);
        setSelectedListing(currentListing);

        // Add a small delay to verify the context update
        setTimeout(() => {
          console.log(
            "📍 Context update verification - selectedListing should now be:",
            currentListing.name
          );
        }, 100);
      }
    }
  }, [currentListing, selectedListing, setSelectedListing, listingsLoading]);

  // Additional useEffect to log context changes
  useEffect(() => {
    console.log(
      "📍 selectedListing context value changed to:",
      selectedListing
    );
  }, [selectedListing]);
  ...
  const generateGridData = (gridSize: string): GridPoint[] => {
    ...
    const centerLng = defaultCoordinates?.lng || 77.209;
    const spacing = 0.003;
      ...
      const rankingIcon = L.divIcon({
        html: `<div style="
          background: ${
            point.ranking === 1
              ? "#22c55e"
              : point.ranking && point.ranking <= 3
              ? "#f59e0b"
              : point.ranking && point.ranking <= 6
              ? "#ef4444"
              : "#ef4444"
          };
          color: white;
      ...
      marker.bindPopup(`
        <div style="text-align: center; padding: 5px;">
          ...
          <button onclick="this.closest('.leaflet-popup').querySelector('.leaflet-popup-close-button').click(); 
            window.removeManualMarker && window.removeManualMarker('${L.stamp(
              marker
            )}')" 
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        <RadioGroup
                          ...
                          onValueChange={(value) =>
                            handleInputChange("searchDataEngine", value)
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            handleInputChange("mapPoint", value)
                          }
                        >
                          <SelectTrigger>
                            <SelectValue />
                        ...
                        {formData.mapPoint === "Manually" && (
                          <p className="text-xs text-blue-600 bg-blue-50 p-2 rounded">
                            Click on the map to place points manually. You can
                      ...
                      <div className="grid lg:grid-cols-2 gap-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleInputChange("distanceValue", value)
                            }
                          >
                            <SelectTrigger>
                              <SelectValue />
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 lg:gap-4">
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleInputChange("gridSize", value)
                            }
                        ...
                        <div className="space-y-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleInputChange("scheduleCheck", value)
                            }
                          >
                            <SelectTrigger>
                              <SelectValue />
