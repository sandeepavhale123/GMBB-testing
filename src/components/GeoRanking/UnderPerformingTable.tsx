...
export const UnderPerformingTable: React.FC<UnderPerformingTableProps> = ({
  ...
  const handleOptimize = (areaId: string) => {
    console.log('Optimizing area:', areaId);
    toast({
      title: "Optimization Started",
      description: "Area optimization process has been initiated"
    });
  };
      ...
      <CardContent className="pt-0">
        {loading ? <div className="flex items-center justify-center py-8">
            ...
            <div className="min-w-[1100px]">
              {/* Header */}
              <div className="grid grid-cols-8 gap-4 pb-3 border-b border-gray-200 mb-2">
                ...
                <div className="font-medium text-gray-700 text-xs sm:text-sm text-right">Action</div>
              </div>
              
              {/* Data Rows */}
              <div className="space-y-2">
                {underPerformingAreas.length > 0 ? underPerformingAreas.map(area => <div key={area.id} className="grid grid-cols-8 gap-4 py-3 px-3 rounded-lg hover:bg-gray-50 transition-colors items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-gray-200" onClick={() => handleCopyCoordinate(area.coordinate)}>
                      ...
                      <div className="text-right">
                        <Button variant="outline" size="sm" className="text-xs" onClick={() => handleOptimize(area.id)}>
                          Optimize
                        </Button>
                      </div>
