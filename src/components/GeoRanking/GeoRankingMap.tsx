...
export const GeoRankingMap: React.FC<GeoRankingMapProps> = ({
  ...
  const { t } = useI18nNamespace("GeoRanking/geoRankingMap");
  ...
  const addDefaultMarker = (): void => {
    ...
    marker.bindPopup(`
      <div style="text-align: center; padding: 5px;">
        <strong>${t("geoRankingMap.defaultMarker.title")}</strong><br>
        <small>${t("geoRankingMap.defaultMarker.manualNote")}</small>
      </div>
    `);
  ...
  const addAutomaticMarkers = (): void => {
    ...
    const gridMarkers = currentMarkers.filter(
      (marker) => (marker.options as any).className !== "default-marker"
    );
    ...
    const defaultMarker = currentMarkers.find(
      (marker) => (marker.options as any).className === "default-marker"
    );

    const gridData =
      gridCoordinates.length > 0 ? generateGridDataFromAPI() : [];
    ...
    if (!defaultMarker) {
      ...
      const defaultMarkerInstance = L.marker(
        [defaultCoordinates.lat, defaultCoordinates.lng],
        {
          icon: defaultIcon,
        }
      ...
      defaultMarkerInstance.bindPopup(`
        <div style="text-align: center; padding: 5px;">
          <strong>${t("geoRankingMap.defaultMarker.title")}</strong><br>
          <small>${t("geoRankingMap.defaultMarker.centerNote")}</small>
        </div>
      `);
    ...
    gridData.forEach((point) => {
      ...
      marker.bindPopup(
        t("geoRankingMap.gridMarker", { id: point.id })
        // `Grid Point: ${point.id}`
      );
      markers.push(marker);
    });
  ...
  const enableManualSelection = (): void => {
    ...
    const handleMapClick = (e: L.LeafletMouseEvent) => {
      ...
      marker.bindPopup(`
        <div style="text-align: center; padding: 5px;">
          <strong>${t("geoRankingMap.manualMarker.title")}</strong><br>
          <small>${t("geoRankingMap.manualMarker.dragNote")}</small><br>
          <button onclick="this.closest('.leaflet-popup').querySelector('.leaflet-popup-close-button').click(); 
            ...
            ${t("geoRankingMap.manualMarker.removeButton")}
          </button>
        </div>
      `);
    ...
    <Card className="overflow-hidden h-[400px] sm:h-[500px] lg:h-[680px]">
      <CardHeader className="pb-3 lg:pb-4">
        <CardTitle className="text-base lg:text-lg font-semibold text-gray-900 flex items-center gap-2">
          ...
          {mapPoint === "Manually"
            ? t("geoRankingMap.manualTitle")
            : t("geoRankingMap.automaticTitle")}
        </CardTitle>
      </CardHeader>
      <CardContent className="p-0 h-full relative">
          ...
          <div className="absolute inset-0 bg-white/90 backdrop-blur-sm flex items-center justify-center z-50 rounded-lg">
            <div className="text-center bg-white p-6 rounded-lg shadow-lg border">
              ...
              <p className="text-sm font-medium text-gray-900 mb-1">
                {t("geoRankingMap.loading.main")}
              </p>
              <p className="text-xs text-gray-500">
                {mapPoint === "Automatic"
                  ? t("geoRankingMap.loading.automatic")
                  : t("geoRankingMap.loading.manual")}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="absolute top-20 right-4 z-20 flex flex-col gap-2">
          ...
          <Button
            ...
            className="w-10 h-10 p-0 bg-white/90 hover:bg-white border shadow-lg"
            title={t("geoRankingMap.zoom.out")}
          >
