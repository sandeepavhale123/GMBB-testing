...
export const useKeywordPolling = (
  ...
  const handleEmptyKeywords = useCallback(async () => {
    console.log(`🔄 [${new Date().toISOString()}] handleEmptyKeywords - Processing completed, refreshing keywords list`);
    ...
    try {
      ...
      console.log(`✅ [${new Date().toISOString()}] Keywords list refreshed successfully after processing completion`);
    } catch (error) {
      console.error(`❌ [${new Date().toISOString()}] Failed to refresh keywords after processing completion:`, error);
    }
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      console.log(`🔄 [${new Date().toISOString()}] checkStatus - Calling /check-keyword-status API for listingId: ${listingId}`);
      ...
      if (response.code === 200) {
        ...
        console.error(`❌ [${new Date().toISOString()}] checkStatus - API error:`, response.message);
      }
    } catch (error) {
      console.error(`❌ [${new Date().toISOString()}] checkStatus - Network/API error:`, error);
    }
  ...
  const checkInitialStatus = useCallback(async () => {
    ...
    console.log(`🚀 [${new Date().toISOString()}] checkInitialStatus - Making initial /check-keyword-status request (listingId: ${listingId})`);
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        console.error(`❌ [${new Date().toISOString()}] checkInitialStatus - API error:`, response.message);
      }
    } catch (error) {
      console.error(`❌ [${new Date().toISOString()}] checkInitialStatus - Network/API error:`, error);
    }
  ...
  const startPolling = useCallback(() => {
    ...
    console.log(`🔄 [${new Date().toISOString()}] startPolling - Starting 5-second polling interval`);
    setIsPolling(true);
    
    pollingIntervalRef.current = setInterval(async () => {
      console.log(`⏰ [${new Date().toISOString()}] Polling interval - checking keyword status`);
      await checkStatus();
    }, 5000);
  }, [checkStatus]);
  ...
  const stopPolling = useCallback(() => {
    console.log(`🛑 [${new Date().toISOString()}] stopPolling - Stopping polling interval`);
    setIsPolling(false);
    clearPollingInterval();
  }, [clearPollingInterval]);
