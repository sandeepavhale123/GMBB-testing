...
export const useChat = (keywordId?: string) => {
  ...
  const transformChatHistory = useCallback(
    (chatItems: ChatHistoryItem[]): ChatSession[] => {
  ...
  const submitFeedback = useCallback(
    async (messageId: string, feedback: FeedbackType) => {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === messageId ? { ...msg, isSubmittingFeedback: true } : msg
        )
      );
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === messageId ? { ...msg, isSubmittingFeedback: false } : msg
          )
        );
  ...
  const transformChatMessages = useCallback(
    (chatMessages: ChatMessageItem[]): ChatMessage[] => {
      return chatMessages
        .map((item, index) => {
          let messageContent = "";
    ...
    async (sessionId: string) => {
      ...
      try {
        ...
        if (response.code === 200 && response.data.chat_messages) {
          const transformedMessages = transformChatMessages(
  ...
  const deleteChatHistory = useCallback(
    async (sessionId: string) => {
      ...
      try {
        ...
        setChatHistory((prev) =>
          prev.filter((chat) => chat.chat_session_id !== sessionId)
        );
