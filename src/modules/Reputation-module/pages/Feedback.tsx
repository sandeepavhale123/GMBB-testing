...
import { Search, Download, Star, ChevronLeft, ChevronRight, MessageCircle, Clock, CheckCircle, Eye } from "lucide-react";
...
export const Feedback: React.FC = () => {
  const {
    t
  } = useTranslation("Reputation/feedback");
  ...
  const filteredData = feedbackData.filter(feedback => {
    const matchesSearch = feedback.customerName.toLowerCase().includes(searchQuery.toLowerCase()) || feedback.message.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleSaveChanges = () => {
    if (selectedFeedback) {
      setFeedbackData(prev => prev.map(f => f.id === selectedFeedback.id ? {
        ...f,
  ...
  const handleExportCSV = () => {
    ...
    const csvContent = [headers.join(","), ...filteredData.map(f => [f.customerName, f.rating, `"${f.message}"`, f.channel, f.submittedDate, f.status].join(","))].join("\n");
  ...
  return <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div className="lg:col-span-2">
              <div className="relative">
                ...
                <Input placeholder={t("filters.search")} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            </div>
      ...
      <Card>
        <CardContent className="p-0">
            ...
            <Table>
              ...
              <TableBody>
                {paginatedData.length === 0 ? <TableRow>
                  ...
                  </TableRow> : paginatedData.map(feedback => <TableRow key={feedback.id} className="hover:bg-muted/50">
          ...
          {filteredData.length > 0 && <div className="flex items-center flex-col md:flex-row justify-between px-6 py-4 border-t gap-4">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedFeedback && <div className="space-y-6">
              <div>
              ...
              <div>
                ...
                <Textarea placeholder={t("modal.adminNotesPlaceholder")} value={modalNotes} onChange={e => setModalNotes(e.target.value)} rows={4} />
              </div>
            </div>}
