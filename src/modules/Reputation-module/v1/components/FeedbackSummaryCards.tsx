...
export const FeedbackSummaryCards: React.FC<FeedbackSummaryCardsProps> = ({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      return <Card key={stat.title} className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-8">{stat.title}</p>
                {isLoading ? <Skeleton className="h-8 w-16 mt-2" /> : <p className="text-3xl font-bold">
                    {stat.value}
                    {stat.suffix && <span className="text-sm text-muted-foreground ml-1">
                        {stat.suffix}
                      </span>}
                  </p>}
              </div>
      ...
      <Card className="p-4">
        ...
        {isLoading ? <div className="space-y-2">
          ...
          return <div key={item.label} className="flex items-center justify-between">
