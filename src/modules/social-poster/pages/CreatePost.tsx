...
import { useUploadMedia, useDeleteMedia, useCreatePost, useUpdatePost, useAvailableAccounts } from "../hooks/useSocialPoster";
import { Post } from "../types";
import { MediaItem } from "../types";
import { useProfile } from "@/hooks/useProfile";
import { convertUTCToUserTimezone, convertUserTimezoneToUTC } from "@/utils/dateUtils";
...
export const SocialPosterCreatePost: React.FC<CreatePostProps> = ({
  ...
  const [selectedAccounts, setSelectedAccounts] = useState<string[]>(editMode && postData ? postData.targets.map((t: any) => t.socialAccountId) : []);
  const {
    profileData
  } = useProfile();
  const userTimezone = profileData?.timezone || "UTC";
  const [scheduleEnabled, setScheduleEnabled] = useState(editMode && postData ? !!postData.scheduledFor : false);
  const [scheduledDate, setScheduledDate] = useState(editMode && postData && postData.scheduledFor ? (() => {
    const zonedDate = convertUTCToUserTimezone(postData.scheduledFor, userTimezone);
    return zonedDate.toISOString().split('T')[0];
  })() : "");
  const [scheduledTime, setScheduledTime] = useState(editMode && postData && postData.scheduledFor ? (() => {
    const zonedDate = convertUTCToUserTimezone(postData.scheduledFor, userTimezone);
    return zonedDate.toISOString().split('T')[1].substring(0, 5);
  })() : "");
  const [platformOptions, setPlatformOptions] = useState<Post["platformOptions"]>(editMode && postData ? postData.platformOptions || {} : {});
  const [showEmojiPicker, setShowEmojiPicker] = useState(false);
  const [showLinkDialog, setShowLinkDialog] = useState(false);
  const [showPreviewModal, setShowPreviewModal] = useState(false);
  const [linkUrl, setLinkUrl] = useState("");
  const [linkText, setLinkText] = useState("");
  const [uploadedMedia, setUploadedMedia] = useState<MediaItem[]>(editMode && postData && postData.media ? postData.media.map((m: any) => ({
    id: m.id,
    mediaUrl: m.mediaUrl,
  ...
  const selectedPlatforms = useMemo(() => {
    const platforms = allAccounts.filter(account => selectedAccounts.includes(account.id)).map(account => account.platform);
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (uploadedMedia.length > 0) {
        toast.error("You can only upload 1 video or 1 image");
        e.target.value = "";
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    if (minLimit !== Infinity && content.length > minLimit) {
      toast.error(`Content exceeds character limit of ${minLimit} characters`);
      return;
    }
    ...
    try {
      ...
      if (scheduleEnabled && scheduledDate && scheduledTime) {
        ...
        const utcDateTime = convertUserTimezoneToUTC(localDateTime, userTimezone);
        scheduledFor = utcDateTime.toISOString();
      }
      const mediaIds = uploadedMedia.map(m => m.id).filter((id): id is string => id !== undefined);
      if (editMode && postId) {
  ...
  return <div className="w-full space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{editMode ? "Edit Post" : "Create Post"}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <div className="lg:col-span-8 space-y-6">
          <SelectAccountsCard selectedAccounts={selectedAccounts} onSelectionChange={setSelectedAccounts} />

          <ComposePostCard content={content} onContentChange={setContent} selectedPlatforms={selectedPlatforms} uploadedMedia={uploadedMedia} isUploading={isUploading} deletingMediaId={deletingMediaId} onMediaUpload={handleMediaUpload} onMediaRemove={removeMedia} onEmojiClick={() => setShowEmojiPicker(true)} onLinkClick={() => setShowLinkDialog(true)} />

          <ScheduleOptionsCard enabled={scheduleEnabled} onEnabledChange={setScheduleEnabled} scheduledDate={scheduledDate} scheduledTime={scheduledTime} onDateChange={setScheduledDate} onTimeChange={setScheduledTime} />
        ...
        <div className="hidden lg:block lg:col-span-4">
          <LivePreviewPanel content={content} media={uploadedMedia} selectedPlatforms={selectedPlatforms} />
        </div>
      </div>
      ...
      <Dialog open={showEmojiPicker} onOpenChange={setShowEmojiPicker}>
        <DialogContent>
          ...
          <div className="grid grid-cols-8 gap-2">
            {emojis.map(emoji => <Button key={emoji} variant="ghost" size="lg" className="text-2xl h-12 w-12" onClick={() => insertEmoji(emoji)}>
                {emoji}
              </Button>)}
          </div>
        </DialogContent>
