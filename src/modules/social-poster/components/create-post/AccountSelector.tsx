...
import { Facebook, Instagram, Twitter, Linkedin, ChevronDown, ChevronRight, MessageSquare } from "lucide-react";
...
export const AccountSelector: React.FC<AccountSelectorProps> = ({ 
  ...
  const [internalSelectedAccounts, setInternalSelectedAccounts] = React.useState<string[]>([]);
  ...
  const selectAllForPlatform = (platform: PlatformType) => {
    ...
    const allSelected = platformAccounts.every(id => selectedAccounts.includes(id));
  ...
  if (platforms.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Target Accounts</CardTitle>
          <CardDescription>Select platforms first</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground text-center py-4">
            Choose platforms to see available accounts
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2 max-h-96 overflow-y-auto">
          {Object.entries(filteredGroupedAccounts).map(([platform, accounts]) => {
                  ...
                  <CollapsibleContent>
                    <CardContent className="pt-0 space-y-2">
                      {accounts.map((account) => (
                        <div key={account.id} className="flex items-center space-x-2 p-2 rounded hover:bg-muted">
                          <Checkbox
                            id={account.id}
