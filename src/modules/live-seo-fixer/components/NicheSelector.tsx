...
export const NicheSelector: React.FC<NicheSelectorProps> = ({
  ...
  const allNiches = React.useMemo(() => {
    ...
    const niches: Array<{ value: string; category: string; subcategory?: string }> = [];
    ...
    if (schemaTypesData.data.LocalBusiness) {
      schemaTypesData.data.LocalBusiness.General?.forEach(type => {
        niches.push({ value: type, category: 'LocalBusiness', subcategory: 'General' });
      });
      schemaTypesData.data.LocalBusiness.Specific?.forEach(type => {
        niches.push({ value: type, category: 'LocalBusiness', subcategory: 'Specific' });
      });
      schemaTypesData.data.LocalBusiness.Retail?.forEach(type => {
        niches.push({ value: type, category: 'LocalBusiness', subcategory: 'Retail' });
      });
    }
  ...
  const renderNichesByCategory = () => {
    ...
    return categories.map(category => {
      const categoryNiches = filteredNiches.filter(n => n.category === category);
      ...
      if (category === 'LocalBusiness') {
        ...
        return (
          <div key={category} className="space-y-2">
            <div className="font-semibold text-sm text-foreground px-2 py-1">{category}</div>
            {subcategories.map(subcategory => {
              const subNiches = categoryNiches.filter(n => n.subcategory === subcategory);
              if (subNiches.length === 0) return null;
              
              return (
                <div key={subcategory} className="pl-4 space-y-1">
                  <div className="text-xs font-medium text-muted-foreground px-2 py-1">{subcategory}</div>
                  {subNiches.map(niche => {
        ...
        <CollapsibleContent className="mt-2">
          <div className="border rounded-md p-4 space-y-4 bg-background">
            ...
            <div className="max-h-[250px] overflow-y-auto space-y-4">
              {isLoading ? (
                <div className="text-sm text-muted-foreground text-center py-4">Loading niches...</div>
              ) : filteredNiches.length === 0 ? (
                <div className="text-sm text-muted-foreground text-center py-4">No niches found</div>
              ) : (
