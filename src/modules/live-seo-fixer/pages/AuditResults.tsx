...
export const AuditResults: React.FC = () => {
  ...
  const handleViewPageAudit = (pageId: string) => {
    ...
    navigate(`/module/live-seo-fixer/projects/${projectId}/pages/${numericId}/audit`);
  };
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-2 flex-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Badge variant="outline">Target: {result.target_keyword}</Badge>
                    <Badge className={getSeverityColor(result.status)}>
                      {getSeverityIcon(result.status)}
                      <span className="ml-1 capitalize">{result.status === 'has_issues' ? 'Issues Found' : result.status}</span>
                    </Badge>
                  </div>
                </div>
                <Button onClick={() => handleViewPageAudit(result.page_id)} className="ml-4">
            ...
            <CardContent>
              <div className="flex items-center gap-6">
                ...
                <div className="text-sm">
                  ...
                  <span className="ml-2 capitalize">{result.status === 'has_issues' ? 'Has Issues' : result.status}</span>
                </div>
              </div>
            </CardContent>
          </Card>
