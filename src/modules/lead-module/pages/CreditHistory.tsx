...
const CreditHistory: React.FC = () => {
  const { t } = useI18nNamespace("Lead-module-pages/creditHistory");
  ...
  const usagePercentage =
    allowedCredit > 0 ? (usedCredits / allowedCredit) * 100 : 0;
  ...
  const transactions =
    creditHistoryData?.data?.history?.map(transformTransaction) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("creditUsageTitle")}</CardTitle>
          <CardDescription>{t("creditUsageDescription")}</CardDescription>
        </CardHeader>
          ...
          <div className="space-y-2">
            {creditsLoading ? (
              ...
              <>
                <div className="flex justify-between text-sm">
                  <span>
                    {t("usedLabel", { used: usedCredits.toLocaleString() })}
                    {/* Used: {usedCredits.toLocaleString()} credits */}
                  </span>
                  <span>
                    {t("availableLabel", {
                      available: remainingCredit.toLocaleString(),
                    })}
                    {/* Available: {remainingCredit.toLocaleString()} credits */}
                  </span>
                </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-8">
              <p className="text-muted-foreground">{t("noHistoryTitle")}</p>
              <p className="text-sm text-muted-foreground">
                {searchTerm ? t("noSearchResults") : t("noHistoryDescription")}
              </p>
            </div>
          ) : (
            <>
                  ...
                  <Pagination>
                    ...
                    <PaginationContent>
                      ...
                      {(() => {
                        ...
                        if (totalPages <= 5) {
                          // Case 1: If total pages <= 5 → show all
                          for (let p = 1; p <= totalPages; p++)
                            pagesToShow.push(p);
                        } else if (currentPage <= 3) {
                          // Case 2: Near start → 1-5
                          pagesToShow.push(1, 2, 3, 4, 5);
                        } else if (currentPage >= totalPages - 2) {
                          // Case 3: Near end → last 5
                          pagesToShow.push(
                            totalPages - 4,
                            totalPages - 3,
                            totalPages - 2,
