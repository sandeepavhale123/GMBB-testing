      ...
      <CardContent className="space-y-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-1 mb-2">
                ...
                <span className="text-sm font-medium">
                  {yourBusiness.averageRating}
                </span>
                <span className="text-sm text-muted-foreground">
                  ({yourBusiness.reviewCount} {t("top20.review")})
                </span>
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 border rounded-lg">
            <div className="text-2xl font-bold text-blue-600">
              {competitorStats.totalCompetitors}
            </div>
            <div className="text-sm text-muted-foreground">
              {t("top20.totalCompetitors")}
            </div>
          </div>
          <div className="text-center p-4 border rounded-lg">
            <div className="text-2xl font-bold text-green-600">
              #{competitorStats.yourPosition}
            </div>
            <div className="text-sm text-muted-foreground">
              {t("top20.yourRank")}
            </div>
          ...
          <div className="text-center p-4 border rounded-lg">
            <div className="text-2xl font-bold text-purple-600">
              {competitorStats.averageRating.toFixed(1)}
            </div>
            <div className="text-sm text-muted-foreground">
              {t("top20.avgCompetitorRating")}
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6">
          <h3 className="font-semibold text-lg mb-4 flex items-center gap-2">
            ...
            {t("top20.comparisonAnalysis")}
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full border-collapse border border-gray-200 rounded-lg">
              <thead>
                <tr className="bg-gray-50">
                  <th className="border border-gray-200 p-3 text-left font-medium text-sm">
                    {t("top20.rank")}
                  </th>
                  <th className="border border-gray-200 p-3 text-left font-medium text-sm">
                    {t("top20.businessName")}
                  </th>
                  <th className="border border-gray-200 p-3 text-left font-medium text-sm">
                    {t("top20.category")}
                  </th>
                      ...
                      <td className="border border-gray-200 p-3 text-sm font-medium">
                        ...
                        {isYourBusiness && (
                          <span className="ml-2 text-blue-600 font-semibold">
                            {t("top20.yourBusiness")}
                          </span>
                        )}
                      </td>
                      <td className="border border-gray-200 p-3 text-sm">
                        {business.category}
                        {business.additionalCategory &&
                          business.additionalCategory !== "Not Found" && (
